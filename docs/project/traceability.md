...
|---|---|---|---|---|
| REQ-CORE-001 | architecture overview §5 and proposed M2.2 spec | Python promotion gate; durable writer not implemented | `tests/test_promotion.py`, Authority Test; M2.2 transaction fixtures planned | partial |
| REQ-CORE-002 | architecture overview §4 and proposed M2.2 authority boundary | module/trust-plane boundaries; writer not implemented | Authority Test plus architecture review | partial |
| REQ-CORE-003 | M1.1/M2.1 specs, ADR 0009, and proposed M2.2 spec | Python/Rust protocol modules, disabled M2 stage-one through stage-eleven pipeline, sealed result types, private conformance history, and distinct non-authorizing public result APIs | shared v1 conformance plus all 252 frozen M2 cases; M2.2 schemas/fixtures planned | partial |
...
| REQ-PROM-002 | lineage architecture and proposed M2.2 spec | append-only Python ledger; SQLite rejected-decision persistence not implemented | rejected Authority Test lineage; M2.2 fixtures planned | partial |
| REQ-LINE-001 | data model and M1.1 | artifact store/canonical hashing | artifact and conformance tests | implemented |
| REQ-LINE-002 | event-sourced lineage ADR and proposed M2.2 spec/ADR 0011 | Python hash-chain ledger; transactional store not implemented | tamper and JSONL-framing tests; SQLite fault fixtures planned | partial |
| REQ-ROLL-001 | constitution v1 and proposed M2.2 reservation contract | Python/Rust gate checks; durable reservation not implemented | missing-target tests; M2.2 rollback fixtures planned | partial |
| REQ-ROLL-002 | proposed M2.2 append-only rollback contract | not implemented | M2.3 rollback drill planned | planned |
...
| REQ-OPS-002 | threat model and proposed M2.2 operator writer/rollback boundary | not implemented | future pause/revoke/recovery tests | planned |
| REQ-OPS-003 | security/non-goals and proposed M2.2 no-ambient-effects contract | deny-by-default design | future sandbox, writer-handle, and operator tests | partial |
| REQ-OBS-001 | observatory architecture and proposed M2.2 post-root-finalization hooks | not implemented | future OpenTelemetry contract tests | planned |
...
| REQ-REP-002 | architecture principles and proposed M2.2 replay/rebuild contract | deterministic demo inputs | demo reproducibility tests; SQLite replay/fault fixtures planned | partial |
